...
PauseScreen::PauseScreen(sf::RenderWindow* hwnd, Input* in, GameState* game)
{
	window = hwnd;
	input = in;
	gameState = game;
	...
	pause_texture.loadFromFile("gfx/pauseScreen.png");
	pause_sprite.setTexture(pause_texture);
	pause_sprite.setScale(10, 10);
	...
	UIText[0].text.setString("Resume");
...
void PauseScreen::updateVisualFeedback()
{
	for (int i = 0; i < 2; i++) {
		if (i == selectedItem) {
